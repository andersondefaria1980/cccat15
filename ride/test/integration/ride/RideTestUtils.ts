...
export default class RideTestUtils {
    ...
    public static async createAccount(
        ...
        const accountInput = {
            name: "Jose da Silva",
            email: `josedasilva_${uuid}@gmail.com`,
            cpf: "02976067945",
            isPassenger: isPassenger,
            isDriver: isDriver,
        };
        const response = await axios.post("http://localhost:3001/signup", accountInput).catch((e) => {
            console.log('ERRO')
        }).finally();
        if (!response?.data?.accountId) {
            throw new Error("Account could not be created. Error: " + response?.data?.error);
        }
